...
import "./MathUC.sol";
import "./MarketMathCoreUC.sol";

library MarketApproxPtInUCLib {
        ...
        )
    {
        ...
        */
            ...
            MarketPreCompute memory comp = arg.market.getMarketPreCompute(
                arg.index,
                arg.blockTime
            );
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (netAssetOut >= minAssetOut) {
                    p.guessMax = guess;
                    bool isAnswerAccepted = MathUC.isAGreaterApproxB(
                        netAssetOut,
                        minAssetOut,
                        p.eps
                    );
        ...
        */
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (netAssetToPull <= maxAssetIn) {
                    p.guessMin = guess;
                    bool isAnswerAccepted = MathUC.isASmallerApproxB(
                        netAssetToPull,
                        maxAssetIn,
                        p.eps
                    );
        ...
        )
    {
        ...
        */
            ...
            p.guessMax = MathUC.min(p.guessMax, arg.totalPtIn);
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                uint256 ptNumerator;
                {
                    ...
                    uint256 newTotalAsset = (comp.totalAsset.Uint() -
                        netAssetOut -
                        netAssetToReserve);
                ...
                if (MathUC.isAApproxB(assetNumerator, ptNumerator, p.eps)) {
                    return (
                        guess,
                        arg.index.assetToScy(netAssetOut),
                        arg.index.assetToScy(netAssetToReserve)
        ...
        )
    {
        unchecked {
            ...
            p.guessMin = MathUC.max(p.guessMin, arg.maxPtIn);
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (guess <= netPtPayable) {
                    ...
                    if (MathUC.isASmallerApproxB(netPtIn, arg.maxPtIn, p.eps)) {
                        return (
                            netAssetOut,
                            netPtIn,
                            guess,
        ...
        returns (ApproxParamsPtIn memory res)
    {
        unchecked {
            ...
            res.guessMax = MathUC.min(_approx.guessMax, calcMaxPtIn(totalAsset));
            res.guessOffchain = _approx.guessOffchain;
            res.maxIteration = _approx.maxIteration;
            res.eps = _approx.eps;

            require(
                res.guessMin <= res.guessMax && res.eps <= MathUC.ONE,
                "invalid approx params"
            );
        }
    }
    ...
    function calcSlope(
        ...
        unchecked {
            ...
            int256 part3 = MathUC.IONE.divDown(comp.rateScalar);
...
library MarketApproxPtOutUCLib {
    using MarketMathCoreUC for MarketState;
    using PYIndexLibUC for PYIndex;
    using MathUC for uint256;
    using MathUC for int256;
        ...
        )
    {
        ...
        */
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                (uint256 netAssetIn, uint256 netAssetToReserve) = calcAssetIn(
                    arg.market,
                    comp,
                    guess
                );
        ...
        )
    {
        ...
        */
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (netAssetOut >= minAssetOut) {
                    p.guessMax = guess;
                    if (MathUC.isAGreaterApproxB(netAssetOut, minAssetOut, p.eps)) {
                        return (
                            guess,
                            arg.index.assetToScy(netAssetOut),
                            arg.index.assetToScy(netAssetToReserve)
        ...
        )
    {
        ...
        */
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (MathUC.isAApproxB(ptNumerator, assetNumerator, p.eps)) {
                    return (
                        guess,
                        arg.index.assetToScy(netAssetIn),
                        arg.index.assetToScy(netAssetToReserve)
        ...
        )
    {
        unchecked {
            ...
            p.guessMin = MathUC.max(p.guessMin, arg.maxYtIn);
            ...
            for (uint256 iter = 0; iter < p.maxIteration; ++iter) {
                ...
                if (netScyIn <= arg.maxScyRedeemableFromPY) {
                    p.guessMin = guess;
                    if (MathUC.isASmallerApproxB(netAssetIn, arg.maxYtIn, p.eps)) {
                        return (
                            guess - arg.maxYtIn,
                            arg.maxYtIn,
                            guess,
    ...
    function calcAssetIn(
        ...
        unchecked {
            (int256 assetToAccount, int256 assetToReserve) = market.calcTrade(
                comp,
                netPtOut.Int()
            );
    ...
    function newApproxParamsPtOut(
        ...
        unchecked {
            res.guessMin = _approx.guessMin;
            res.guessMax = MathUC.min(_approx.guessMax, calcMaxPtOut(comp, totalPt));
            res.guessOffchain = _approx.guessOffchain;
            res.maxIteration = _approx.maxIteration;
            res.eps = _approx.eps;
        ...
        returns (uint256)
    {
        unchecked {
            ...
            int256 proportion = logitP.divDown(logitP + MathUC.IONE);
