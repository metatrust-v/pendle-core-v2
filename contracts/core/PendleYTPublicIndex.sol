...
import "../SuperComposableYield/ISCYRewardIndex.sol";
...
contract PendleYieldToken is PendleBaseToken, IPYieldToken, RewardManager {
    ...
    function updateGlobalReward() public virtual {
        _updateGlobalReward(new address[](0), 0);
    }
    ...
    function updateUserReward(address user) public virtual {
        ...
        _updateUserReward(user, impliedScyBalance, 0);
    }

    function _updateUserReward(
        address user,
        uint256 impliedScyBalance,
        uint256
    ) internal virtual override {
        address[] memory rewardTokens = getRewardTokens();
        _updateGlobalReward(rewardTokens, 0);
        _updateUserRewardSkipGlobal(rewardTokens, user, impliedScyBalance);
    }
    ...
    function _updateUserRewardSkipGlobal(
        ...
        for (uint256 i = 0; i < rewardTokens.length; ++i) {
            ...
            uint256 scyRewardIndex = ISCYRewardIndex(SCY).globalReward(token).index;
            uint256 userLastScyIndex = userReward[user][token].lastIndex;

            if (userLastScyIndex == 0) {
                // first time participating reward
                userReward[user][token].lastIndex = scyRewardIndex;
                continue;
            }

            uint256 rewardAmountPerUnit = scyRewardIndex - userLastScyIndex;
            uint256 rewardFromUint = impliedScyBalance.mulDown(rewardAmountPerUnit);

            userReward[user][token].accruedReward += rewardFromUint;
            userReward[user][token].lastIndex = globalReward[token].index;
        }
    }

    function _updateGlobalReward(address[] memory, uint256)
        internal
        virtual
        override
    {
        ISCYRewardIndex(SCY).updateUserReward(address(this));
    }

    function _redeemExternalReward() internal virtual override {
        ISCYRewardIndex(SCY).redeemReward(address(this), address(this));
    }
