...
abstract contract InterestManagerYT is TokenHelper, IPInterestManagerYT {
    ...
    function _distributeInterest(address user, uint256 currentIndex) private {
        ...
        uint256 principal = _interestSharesUser(user);
    ...
    function _interestSharesUser(address user) internal view virtual returns (uint256);

    function _interestSharesTotal() internal view virtual returns (uint256);
}
