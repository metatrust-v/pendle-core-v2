...
contract PendleBenqiLiquidYieldToken is PendleLiquidYieldToken {
    ...
    ) PendleLiquidYieldToken(_name, _symbol, __decimals, _rewardTokens, _yieldToken) {
    ...
    function exchangeRateCurrent() public override returns (uint256) {
        exchangeRateStored = Math.max(exchangeRateStored, IQiToken(yieldToken).exchangeRateCurrent());
        return exchangeRateStored;
    }

    function redeemReward(bool[] memory doRedeem) public override returns (uint256[] memory outAmounts) {
        require(doRedeem.length == rewardTokens.length, "REWARD_TO_REDEEM_MISMATCH");

        updateUserReward(msg.sender);

        for (uint8 i = 0; i < rewardTokens.length; ++i) {
            if (!doRedeem[i]) continue;
    ...
    function updateGlobalReward() public override {
        ...
        for (uint8 i = 0; i < rewardTokens.length; ++i) {
            ...
            if (totalLYT != 0) {
                globalReward[i].index += (currentRewardBalance - globalReward[i].lastBalance).divUp(totalLYT);
            }
