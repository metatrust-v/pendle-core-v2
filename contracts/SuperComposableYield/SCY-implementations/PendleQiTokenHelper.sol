...
contract PendleQiTokenHelper {
    ...
    function _exchangeRateCurrentView() internal view returns (uint256) {
        ...
        simpleInterestFactor = borrowRateMantissa * timestampDelta;

        interestAccumulated = (simpleInterestFactor * borrowsPrior) / 1e18;

        totalBorrowsNew = interestAccumulated + borrowsPrior;

        totalReservesNew = qiToken.reserveFactorMantissa() * interestAccumulated + reservesPrior;

        return _alpha(qiToken.totalSupply(), cashPrior, totalBorrowsNew, totalReservesNew);
    }
