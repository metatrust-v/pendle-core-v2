...
contract QiTokenSaleDistributor is ReentrancyGuard, QiTokenSaleDistributorStorage {
    ...
    function claim() public nonReentrant {
        uint256 availableTokensToClaim = 0;
        for (uint256 round = 0; round < completedPurchaseRounds[msg.sender]; round += 1) {
            uint256 claimableRoundTokens = _getClaimableTokenAmountPerRound(msg.sender, round);
            availableTokensToClaim = availableTokensToClaim.add(claimableRoundTokens);
            claimedTokens[msg.sender][round] = claimedTokens[msg.sender][round].add(
                claimableRoundTokens
            );
        }
     ...
     */
    function getRoundClaimableTokenAmount(uint256 round) public view returns (uint256) {
     ...
     */
    function getClaimedTokenAmount() public view returns (uint256) {
     ...
     * @return Number of claimed QI tokens
     */
    function getRoundClaimedTokenAmount(uint256 round) public view returns (uint256) {
    ...
    function setPurchasedTokensByUser(
        ...
        uint256[] memory tokenInitialReleasePercentages,
        uint256[] memory tokenReleasePeriods,
        uint256[] memory amounts
    ) public adminOrDataAdminOnly {
        ...
        for (uint256 i = 0; i < recipients.length; i += 1) {
            ...
            initialReleasePercentages[recipient][rounds[i]] = tokenInitialReleasePercentages[i]
                .mul(1e18);
            ...
            claimedTokens[recipient][rounds[i]] = tokenInitialReleasePercentages[i]
                .mul(1e18)
                .mul(amounts[i])
                .div(100e18);
        }
    ...
    function resetPurchasedTokensByUser(address[] memory recipients) public adminOrDataAdminOnly {
        for (uint256 i = 0; i < recipients.length; i += 1) {
            ...
            for (uint256 round = 0; round < completedPurchaseRounds[recipient]; round += 1) {
     ...
     */
    function withdrawQi(uint256 amount) public adminOnly {
        EIP20Interface qi = EIP20Interface(qiContractAddress);
     ...
     */
    function _getClaimableTokenAmount(address recipient) internal view returns (uint256) {
        if (completedPurchaseRounds[recipient] == 0) {
            return 0;
        }

        uint256 remainingClaimableTokensToDate = 0;
        for (uint256 round = 0; round < completedPurchaseRounds[recipient]; round += 1) {
            uint256 remainingRoundClaimableTokensToDate = _getClaimableTokenAmountPerRound(
                recipient,
                round
            );
            remainingClaimableTokensToDate = remainingClaimableTokensToDate.add(
                remainingRoundClaimableTokensToDate
            );
        }
     ...
     */
    function _getClaimableTokenAmountPerRound(address recipient, uint256 round)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 initialClaimableTokens = initialReleasePercentages[recipient][round]
            .mul(purchasedTokens[recipient][round])
            .div(100e18);

        uint256 elapsedSecondsSinceEpoch = block.timestamp.sub(vestingScheduleEpoch);
        // Number of elapsed release periods after the initial release
        uint256 elapsedVestingReleasePeriods = elapsedSecondsSinceEpoch.div(releasePeriodLength);

        uint256 claimableTokensToDate = 0;
        if (elapsedVestingReleasePeriods.add(1) >= releasePeriods[recipient][round]) {
            ...
            uint256 claimableTokensPerPeriod = purchasedTokens[recipient][round]
                .sub(initialClaimableTokens)
                .div(releasePeriods[recipient][round].sub(1));
            claimableTokensToDate = claimableTokensPerPeriod.mul(elapsedVestingReleasePeriods).add(
        ...
        uint256 remainingClaimableTokensToDate = claimableTokensToDate.sub(
            claimedTokens[recipient][round]
        );
    ...
    modifier adminOrDataAdminOnly() {
        require(
            msg.sender == admin || (dataAdmin != address(0) && msg.sender == dataAdmin),
            "admin only"
        );
        _;
    }
