    ...
    ExponentialNoError
{
    ...
    event SpeedUpdated(uint8 tokenType, QiToken indexed qiToken, uint256 newSpeed);

    /// @notice Emitted when a new BENQI speed is set for a contributor
    event ContributorQiSpeedUpdated(address indexed contributor, uint256 newSpeed);
    ...
    event NewBorrowCap(QiToken indexed qiToken, uint256 newBorrowCap);

    /// @notice Emitted when borrow cap guardian is changed
    event NewBorrowCapGuardian(address oldBorrowCapGuardian, address newBorrowCapGuardian);

    /// @notice Emitted when BENQI is granted by admin
    event QiGranted(address recipient, uint256 amount);
     ...
     */
    function enterMarkets(address[] memory qiTokens) public override returns (uint256[] memory) {
        uint256 len = qiTokens.length;

        uint256[] memory results = new uint256[](len);
        for (uint256 i = 0; i < len; i++) {
            QiToken qiToken = QiToken(qiTokens[i]);

            results[i] = uint256(addToMarketInternal(qiToken, msg.sender));
        }
     ...
     */
    function exitMarket(address qiTokenAddress) external override returns (uint256) {
        QiToken qiToken = QiToken(qiTokenAddress);
        /* Get sender tokensHeld and amountOwed underlying from the qiToken */
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = qiToken.getAccountSnapshot(
            msg.sender
        );
        ...
        uint256 allowed = redeemAllowedInternal(qiTokenAddress, msg.sender, tokensHeld);
        if (allowed != 0) {
    ...
    function redeemAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(qiToken, redeemer, redeemTokens);
    ...
    function borrowAllowed(
        ...
        uint256 borrowCap = borrowCaps[qiToken];
        // Borrow cap of 0 corresponds to unlimited borrowing
        if (borrowCap != 0) {
            uint256 totalBorrows = QiToken(qiToken).totalBorrows();
    ...
    function liquidateBorrowAllowed(
        ...
        uint256 borrowBalance = QiToken(qiTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function transferAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(qiToken, src, transferTokens);
        if (allowed != uint256(Error.NO_ERROR)) {
            return allowed;
        }
        ...
        )
    {
        (
            ...
            uint256 liquidity,
            uint256 shortfall
        ) = getHypotheticalAccountLiquidityInternal(account, QiToken(address(0)), 0, 0);
    ...
    {
        return getHypotheticalAccountLiquidityInternal(account, QiToken(address(0)), 0, 0);
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.qiTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset
    ...
    function liquidateCalculateSeizeTokens(
        ...
        uint256 priceBorrowedMantissa = oracle.getUnderlyingPrice(QiToken(qiTokenBorrowed));
        uint256 priceCollateralMantissa = oracle.getUnderlyingPrice(QiToken(qiTokenCollateral));
         ...
         */
        uint256 exchangeRateMantissa = QiToken(qiTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint256 seizeTokens;
     ...
     */
    function _setCollateralFactor(QiToken qiToken, uint256 newCollateralFactorMantissa)
        external
        returns (uint256)
    {
        // Check caller is admin
     ...
     */
    function _supportMarket(QiToken qiToken) external returns (uint256) {
        if (msg.sender != admin) {
        ...
        markets[address(qiToken)].isListed = true;
     ...
     */
    function _setMarketBorrowCaps(QiToken[] calldata qiTokens, uint256[] calldata newBorrowCaps)
        external
    {
        require(
        ...
        uint256 numMarkets = qiTokens.length;
    ...
    function setRewardSpeedInternal(
        ...
        uint256 currentRewardSpeed = rewardSpeeds[rewardType][address(qiToken)];
    ...
    function updateRewardSupplyIndex(uint8 rewardType, address qiToken) internal {
        ...
        uint256 supplySpeed = rewardSpeeds[rewardType][qiToken];
        uint256 blockTimestamp = getBlockTimestamp();
        uint256 deltaTimestamps = sub_(blockTimestamp, uint256(supplyState.timestamp));
        if (deltaTimestamps > 0 && supplySpeed > 0) {
            uint256 supplyTokens = QiToken(qiToken).totalSupply();
    ...
    function updateRewardBorrowIndex(
        ...
        uint256 borrowSpeed = rewardSpeeds[rewardType][qiToken];
        uint256 blockTimestamp = getBlockTimestamp();
        uint256 deltaTimestamps = sub_(blockTimestamp, uint256(borrowState.timestamp));
        if (deltaTimestamps > 0 && borrowSpeed > 0) {
            uint256 borrowAmount = div_(QiToken(qiToken).totalBorrows(), marketBorrowIndex);
     ...
     */
    function updateAndDistributeSupplierRewardsForToken(address qiToken, address account)
        internal
    {
    ...
    function grantRewardInternal(
        ...
        if (rewardType == 0) {
            ...
            uint256 qiRemaining = benqi.balanceOf(address(this));
     ...
     */
    function _grantQi(address payable recipient, uint256 amount) public {
