...
/**
 * @title Benqi's QiToken Contract
 * @notice Abstract base for QiTokens
 * @author Benqi
 */
abstract contract QiToken is QiTokenInterface, Exponential, TokenErrorReporter {
    /**
    ...
    function transferTokens(
        ...
        (mathErr, srqiTokensNew) = subUInt(accountTokens[src], tokens);
        if (mathErr != MathError.NO_ERROR) {
        ...
        accountTokens[src] = srqiTokensNew;
    ...
    {
        uint256 qiTokenBalance = accountTokens[account];
        ...
        return (uint256(Error.NO_ERROR), qiTokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    /**
     * @notice User supplies assets into the market and receives qiTokens in exchange
    ...
    function mintFresh(address minter, uint256 mintAmount) internal returns (uint256, uint256) {
         ...
         *  Note: The qiToken must handle variations between ERC-20 and AVAX underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the qiToken holds an additional `actualMintAmount`
         *  of cash.
         */
        vars.actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of qiTokens to be minted:
        ...
        /*
         * We calculate the new total supply of qiTokens and minter token balance, checking for overflow:
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming qiTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice User redeems qiTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of qiTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming qiTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    function repayBorrowFresh(
         ...
         *  Note: The qiToken must handle variations between ERC-20 and AVAX underlying.
         *  On success, the qiToken holds an additional repayAmount of cash.
    ...
    function liquidateBorrowInternal(
        ...
        error = qiTokenCollateral.accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
            return (
                fail(Error(error), FailureInfo.LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED),
                0
            );
        }

        // liquidateBorrowFresh emits borrow-specific logs on errors, so we don't need to
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, qiTokenCollateral);
    }
    ...
    function liquidateBorrowFresh(
        ...
        /* Verify qiTokenCollateral market's block timestamp equals current block timestamp */
        if (qiTokenCollateral.accrualBlockTimestamp() != getBlockTimestamp()) {
            return (
                fail(Error.MARKET_NOT_FRESH, FailureInfo.LIQUIDATE_COLLATERAL_FRESHNESS_CHECK),
                0
            );
        }
        ...
        if (borrower == liquidator) {
            return (
                fail(Error.INVALID_ACCOUNT_PAIR, FailureInfo.LIQUIDATE_LIQUIDATOR_IS_BORROWER),
                0
            );
        }
        ...
        if (repayBorrowError != uint256(Error.NO_ERROR)) {
            return (
                fail(Error(repayBorrowError), FailureInfo.LIQUIDATE_REPAY_BORROW_FRESH_FAILED),
                0
            );
        }
        ...
        require(qiTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint256 seizeError;
        if (address(qiTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, borrower, seizeTokens);
        } else {
            seizeError = qiTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        /* We call the defense hook */
        // unused function
        // comptroller.liquidateBorrowVerify(address(this), address(qiTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another QiToken.
     *  Its absolutely critical to use msg.sender as the seizer qiToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed qiToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of qiTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    ...
    function _addReservesFresh(uint256 addAmount) internal returns (uint256, uint256) {
        ...
        if (accrualBlockTimestamp != getBlockTimestamp()) {
            return (
                fail(Error.MARKET_NOT_FRESH, FailureInfo.ADD_RESERVES_FRESH_CHECK),
                actualAddAmount
            );
        }
