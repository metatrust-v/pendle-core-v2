...
contract PglStakingContract is ReentrancyGuard, PglStakingContractStorage {
     ...
     */
    function deposit(uint256 pglAmount) external nonReentrant {
        ...
        uint256 contractBalance = pglToken.balanceOf(address(this));
        pglToken.transferFrom(msg.sender, address(this), pglAmount);
        uint256 depositedAmount = pglToken.balanceOf(address(this)).sub(contractBalance);
     ...
     */
    function redeem(uint256 pglAmount) external nonReentrant {
        require(pglTokenAddress != address(0), "PGL Token address can not be zero");
    ...
    function claimRewards() external nonReentrant {
        ...
        for (uint256 i = 0; i < nofStakingRewards; i += 1) {
    ...
    function getClaimableRewards(uint256 rewardToken) external view returns (uint256) {
        ...
        uint256 rewardIndexDelta = rewardIndex[rewardToken].sub(
            supplierRewardIndex[msg.sender][rewardToken]
        );
        uint256 claimableReward = rewardIndexDelta.mul(supplyAmount[msg.sender]).div(1e36).add(
            accruedReward[msg.sender][rewardToken]
        );
     ...
     */
    function setRewardSpeed(uint256 rewardToken, uint256 speed) external adminOnly {
     ...
     */
    function setRewardTokenAddress(uint256 rewardToken, address rewardTokenAddress)
        external
        adminOnly
    {
    ...
    function accrueReward() internal {
        uint256 blockTimestampDelta = block.timestamp.sub(accrualBlockTimestamp);
        ...
        for (uint256 i = 0; i < nofStakingRewards; i += 1) {
            ...
            uint256 accruedPerPGL = accrued.mul(1e36).div(totalSupplies);
    ...
    function distributeReward(address recipient) internal {
        ...
        for (uint256 i = 0; i < nofStakingRewards; i += 1) {
            uint256 rewardIndexDelta = rewardIndex[i].sub(supplierRewardIndex[recipient][i]);
            uint256 accruedAmount = rewardIndexDelta.mul(supplyAmount[recipient]).div(1e36);
     ...
     */
    function claimAvax(address payable recipient, uint256 amount) internal {
